...
vbg = cv2.createBackgroundSubtractorMOG2()

while True:
    t, vc = v.read()
    vcmask = vbg.apply(vc)
    cv2.imshow("MOG2", vcmask)
    a, b = cv2.threshold(vcmask, 235, 255, cv2.THRESH_BINARY)
    cv2.imshow("threshold", b)
    # vcmask2 = cv2.morphologyEx(vcmask, cv2.MORPH_CLOSE, np.ones((15, 15)))
    # cv2.imshow("mask-morphologyEx", vcmask)

    # vc2 = cv2.Canny(vcmask, 200, 50)
